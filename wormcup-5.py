...
def decide_score(match):
    ...
    if PREDICT_MODE == "random":
        ...
        # full random, masing-masing tim independen, range 0-4
...
# ===================== MAIN PER ACCOUNT =====================
async def process_account(session_string: str, idx: int, total: int):
    ...
    if saved and is_token_valid(saved["access_token"]):
        ...
        print(f"{tag} token tersimpan dipakai")
    else:
    ...
    print(f"{tag} check-in: {'OK' if ci.get('success') else ci}")
    ...
    for m in matches:
        if m["status"] == "UPCOMING" and m["pool"]["status"] == "OPEN" and m["my_prediction"] is None:
            ...
            status = "OK" if res.get("success") else res
            print(f"{tag} predict {home} {hs}-{as_} {away} -> {status}")
            predicted += 1
    if predicted == 0:
        print(f"{tag} gak ada match buat di-predict")
    ...
    if remaining > 0:
        ...
        print(f"{tag} tap sudah habis hari ini")
...
def select_accounts():
    n = len(SESSIONS)
    print(f"\nTotal akun terdaftar: {n}")
    print("Pilih mode:")
    print("  1. Satu akun")
    print("  2. Semua akun")
    print("  3. Range (dari - sampai)")
    print("  4. Start bot doang (kirim /start, gak predict/tap)")
    choice = input("Pilihan (1/2/3/4): ").strip()

    if choice == "1":
        idx = int(input(f"Index akun (1-{n}): ").strip())
        return [(idx, SESSIONS[idx - 1])], False

    if choice == "3":
        start = int(input(f"Dari index (1-{n}): ").strip())
        end = int(input(f"Sampai index (1-{n}): ").strip())
        return [(i, SESSIONS[i - 1]) for i in range(start, end + 1)], False

    if choice == "4":
        return list(enumerate(SESSIONS, start=1)), True

    # default: semua akun
    return list(enumerate(SESSIONS, start=1)), False


async def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--account", type=int, help="jalankan 1 akun aja (index 1-based)")
    parser.add_argument("--from", dest="from_idx", type=int, help="mulai dari index (1-based)")
    parser.add_argument("--to", dest="to_idx", type=int, help="sampai index (1-based, inclusive)")
    ...
    if args.account or args.from_idx or args.to_idx:
        indexed = list(enumerate(SESSIONS, start=1))
        if args.account:
            indexed = [x for x in indexed if x[0] == args.account]
        else:
            start = args.from_idx or 1
            end = args.to_idx or len(SESSIONS)
            indexed = [x for x in indexed if start <= x[0] <= end]
        start_only = False
    else:
        indexed, start_only = select_accounts()
    ...
    if start_only:
        for idx, s in indexed:
            try:
                client = Client(name=f"acc{idx}", session_string=s, in_memory=True, no_updates=True)
                await client.start()
    ...
    for idx, s in indexed:
        try:
            await process_account(s, idx, total)
        except Exception as e:
            print(f"[Akun {idx}] Error: {e}")
